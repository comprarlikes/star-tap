...
export const GameTipBanner: React.FC<GameTipBannerProps> = ({
  ...
  return (
    <div
      className={`w-full max-w-sm sm:max-w-md mx-auto px-3 py-2 sm:px-3.5 sm:py-2.5 bg-slate-950/85 backdrop-blur-md rounded-2xl border border-amber-500/35 shadow-[0_4px_20px_rgba(245,158,11,0.15)] flex items-center justify-between gap-2 sm:gap-2.5 transition-all duration-300 relative overflow-hidden group select-none shrink-0 ${className}`}
    >
      ...
      {/* Tip Icon Badge */}
      <div className="p-1.5 sm:p-2 rounded-xl bg-gradient-to-tr from-amber-500/20 via-yellow-500/20 to-orange-500/10 border border-amber-400/40 text-amber-300 shadow-sm flex-shrink-0 flex items-center justify-center">
        <Lightbulb className="w-3.5 h-3.5 sm:w-4 sm:h-4 animate-pulse stroke-[2.2]" />
      </div>

      {/* Tip Content Area */}
      <div className="flex-1 flex flex-col min-w-0 text-left">
        <div className="flex items-center gap-1.5 mb-0.5">
          <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-wider text-amber-400/90 font-mono flex items-center gap-1">
            <Sparkles className="w-2.5 h-2.5 text-amber-300" />
            {t('gameTipLabel', safeLang)}
          </span>
          <span className="text-[8px] sm:text-[9px] text-slate-500 font-bold ml-auto font-mono">
            {currentIndex + 1}/{TIP_KEYS.length}
          </span>
        </div>

        <p
          className={`text-[10px] sm:text-[11px] leading-tight sm:leading-snug font-medium text-slate-200 transition-opacity duration-200 line-clamp-2 ${
